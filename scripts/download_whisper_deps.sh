...
pip3 download torch numpy tqdm more-itertools tiktoken setuptools wheel -d whisper_pkgs
if [ $? -ne 0 ]; then
    echo "错误: 依赖包下载失败"
    echo "请检查网络连接或尝试使用国内镜像源"
    echo "例如: pip3 download -i https://pypi.tuna.tsinghua.edu.cn/simple torch numpy tqdm more-itertools tiktoken setuptools wheel -d whisper_pkgs"
    exit 1
fi
...
echo "   然后运行 ./install_whisper_offline.sh 进行离线安装"
echo
echo "4. 如果下载的torch不是你需要的版本，请删除并手动下载正确版本"
echo
echo "5. 给安装脚本添加执行权限: chmod +x install_whisper_offline.sh"
echo