...
pip3 install --no-index --find-links="./whisper_pkgs" torch numpy tqdm more-itertools tiktoken setuptools wheel
if [ $? -ne 0 ]; then
    echo "错误: 依赖包安装失败"
...
if [ -d "backend/whisper-main/whisper-main" ]; then
    ...
    echo "或使用在线安装: pip3 install whisper"
...
echo "验证安装..."
echo "========================================"
python3 -c "import torch; import whisper; print('✓ 所有依赖安装成功!'); print(f'✓ PyTorch 版本: {torch.__version__}'); print(f'✓ CUDA 可用: {torch.cuda.is_available()}')"
if [ $? -ne 0 ]; then
...
echo "运行 python3 test_local_whisper.py 来测试本地Whisper功能"
echo