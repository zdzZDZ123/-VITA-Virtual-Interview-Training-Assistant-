...
import { DigitalHumanInterviewRoom } from './components/digital-human/DigitalHumanInterviewRoom';
import { DoubaoRealtimeVoice } from './components/voice/DoubaoRealtimeVoice';
import './index.css';
...
function App() {
  ...
  const [interviewMode, setInterviewMode] = useState<'text' | 'voice' | 'doubao' | 'digital'>('text');
  const { startSession, submitAnswer, currentQuestion, isLoading, error } = useInterviewStore();

  const handleStartInterview = async (mode: 'text' | 'voice' | 'doubao' | 'digital') => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : '启动面试失败';
          ...
          <div style={{
                ...
                {[
                  ...
                  >
                    <input
                      ...
                      onChange={(e) => setInterviewType(e.target.value as 'behavioral' | 'technical' | 'situational')}
                      style={{ display: 'none' }}
                    />
            ...
            <div style={{ 
              ...
              {[
                ...
                { mode: 'voice', icon: '🎙️', title: '语音面试', desc: '本地语音交互', color: '#2196F3' },
                { mode: 'doubao', icon: '🚀', title: '实时语音面试', desc: '智能语音对话', color: '#9C27B0', highlight: true },
                { mode: 'digital', icon: '🤖', title: '数字人面试', desc: '3D虚拟面试官', color: '#FF5722' }
              ].map((option) => (
                <button
                  key={option.mode}
                  onClick={() => handleStartInterview(option.mode as 'text' | 'voice' | 'doubao' | 'digital')}
                  disabled={isLoading}
                  style={{
                    padding: '1.5rem 1rem',
                    border: option.highlight ? '3px solid #9C27B0' : '2px solid #e0e0e0',
                    borderRadius: '0.5rem',
                    background: option.highlight ? 'linear-gradient(135deg, #f3e5f5 0%, #e1bee7 100%)' : 'white',
                  ...
                  onMouseEnter={(e) => {
                    if (!isLoading) {
                      ...
                      target.style.borderColor = option.color;
                      target.style.background = option.highlight 
                        ? 'linear-gradient(135deg, #f3e5f5 0%, #e1bee7 50%, #ce93d8 100%)'
                        : option.color + '10';
                      target.style.transform = 'translateY(-2px)';
                      target.style.boxShadow = '0 6px 24px rgba(0,0,0,0.15)';
                    }
                  }}
                  onMouseLeave={(e) => {
                    if (!isLoading) {
                      const target = e.target as HTMLButtonElement;
                      target.style.borderColor = option.highlight ? '#9C27B0' : '#e0e0e0';
                      target.style.background = option.highlight 
                ...
                >
                  ...
                  <div style={{ fontSize: '2.5rem', marginBottom: '0.8rem' }}>{option.icon}</div>
                  <div style={{ fontWeight: '600', color: '#333', marginBottom: '0.5rem', fontSize: '0.95rem' }}>
                    {option.title}
                  </div>
                  <div style={{ fontSize: '0.8rem', color: '#666' }}>{option.desc}</div>
                </button>
              ))}
            </div>
  ...
  // 豆包实时语音面试 - 全屏模式
  if (interviewMode === 'doubao') {
    return (
      <DoubaoRealtimeVoice 
        sessionId={`vita-${Date.now()}`}
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f5f5f5' }}>
      <div style={{
        ...
        <div style={{ 
          ...
          <h1 style={{ fontSize: '1.5rem', fontWeight: '600', color: '#333' }}>
            ...
            {interviewMode === 'voice' && '🎙️ 语音面试'}
            {interviewMode === 'doubao' && '🚀 实时语音面试'}
      ...
      <div style={{ padding: '2rem', maxWidth: '800px', margin: '0 auto' }}>
        ...
        {interviewMode === 'voice' && (
          <VoiceInterviewer
            question={currentQuestion || '正在加载问题...'}
            onTranscription={handleSubmitAnswer}
            isVoiceMode={true}
          />
        )}
        {interviewMode === 'digital' && (
          <DigitalHumanInterviewRoom />
        )}
      </div>
    </div>
  );
