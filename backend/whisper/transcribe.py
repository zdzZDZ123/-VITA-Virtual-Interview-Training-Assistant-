...
from core.timing import add_word_timestamps
...
def cli():
    ...
    parser.add_argument("__REMOVED_API_KEY__", type=optional_float, default=0.2, help="temperature to increase when falling back when the decoding fails to meet either of the thresholds below")
    ...
    parser.add_argument("__REMOVED_API_KEY__", type=optional_float, help="(requires --word_timestamps True) skip silent periods longer than this threshold (in seconds) when a possible hallucination is detected")
    # fmt: on
    ...
    if (increment := args.pop("__REMOVED_API_KEY__")) is not None:
